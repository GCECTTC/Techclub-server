  ...
  firebase
    ...
    .catch(function (error) {
      ...
      if (errorCode == "auth/weak-password") {
        return res.json({
          error: true,
          message: "The password is too weak. Use minimum 6 digit",
        })
      } else {
          ...
          admin
            ...
            .then(data => {
              if (data.empty) {
                ...
                req.user = {
                  isActivated: data.docs[0].data().isActivated,
    ...
    admin
      ...
      .then(d => {
        if (d.empty) {
          ...
          req.token = idToken
          req.user = {
            isActivated: d.docs[0].data().isActivated,
            name: data.name,
            email: data.email,
            uid: data.uid,
            success: true,
...
exports.checkCustomUser = (req, res, next) => {
  ...
  let idToken
  if (myTestCookie && myTestCookie.startsWith("Bearer ")) {
  ...
  admin
    ...
    .catch(function (error) {
      console.error("error while verifing token")
